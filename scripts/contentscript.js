...
async function get_first_uid_and_cursor(cursor) {
    ...
    const __aaid = /",\[],{"AD_ACCOUNT_ID":"([^"]+)"},/g.exec(document.body.innerHTML)[1];
        ...
        .then(async (data) => {
            ...
            for (const url of scripts_) {
                await fetch(url[1])
                    ...
                    .then((text) => {
                        let doc_id_regex_test;
                        if (is_private) {
                            doc_id_regex_test = /__d\("GroupsCometMembersPageNewMembersSectionRefetchQuery_facebookRelayOperation",\[],\(function\(a,b,c,d,e,f\)\{e\.exports="([A-Za-z0-9]+)"\}\),null\);/.exec(text);
                        } else {
                            doc_id_regex_test = /__d\("GroupsCometMembersPageNewForumMembersSectionRefetchQuery_facebookRelayOperation",\[],\(function\(a,b,c,d,e,f\)\{e\.exports="([A-Za-z0-9]+)"\}\),null\);/.exec(text);
                        }
...
function get_uid(progress, first_user_list, all_users_uid, cursor, group_id_and_id, doc_id, lsd, __aaid, __spin_t, __spin_b, __rev_and__spin_r, fb_dtsg, __hsi, jazoest, __comet_req, __user, __a, limit, is_private) {

    var payload;
    if (is_private == true) {
        payload = `av=${__user}&__user=${__user}&__a=${__a}&__dyn=&__csr=&__req=&__hs=19276.HYP%3Acomet_pkg.2.1.0.2.1&dpr=1.5&__ccg=EXCELLENT&__rev=${__rev_and__spin_r}&__s=&__hsi=${__hsi}&__comet_req=${__comet_req}&fb_dtsg=${fb_dtsg}&jazoest=${jazoest}&lsd=${lsd}&__aaid=${__aaid}&__spin_r=${__rev_and__spin_r}&__spin_b=${__spin_b}&__spin_t=${__spin_t}&fb_api_caller_class=RelayModern&fb_api_req_friendly_name=GroupsCometMembersPageNewForumMembersSectionRefetchQuery&variables=%7B%22count%22%3A10%2C%22cursor%22%3A%22${cursor}%22%2C%22groupID%22%3A%22${group_id_and_id}%22%2C%22recruitingGroupFilterNonCompliant%22%3Afalse%2C%22scale%22%3A1%2C%22id%22%3A%22${group_id_and_id}%22%7D&server_timestamps=true&doc_id=${doc_id}`;
    } else {
    ...
    return new Promise((resolve, reject) => fetch(myRequest, myOptions)
        .then(response => {
            ...
            response.json().then(json => {
                try {
                    var new_forum_members = json.data.node.new_forum_members || json.data.node.new_members;
                ...
                if (new_forum_members != undefined && new_forum_members.page_info.has_next_page == true && Object.keys(first_user_list).length < limit) {
                    ...
                    get_uid(progress, first_user_list, all_users_uid, cursor, group_id_and_id, doc_id, lsd, __aaid, __spin_t, __spin_b, __rev_and__spin_r, fb_dtsg, __hsi, jazoest, __comet_req, __user, __a, limit, is_private)
                        .then(resolve)
                        .catch(reject)
